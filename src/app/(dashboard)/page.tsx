...
export default function DashboardPage() {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      // 1. Get fresh ID token to satisfy Security Rules
      ...
      if (!res.ok) {
        // Handle the 500/Permission Denied errors gracefully
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium">Welcome back, {(firebaseUser as any)?.name || "Member"}</p>
        </div>
        <Link 
          href="/invitations"
      ...
      <div className="space-y-4">
        ...
        {groups.length === 0 ? (
          <div className="bg-gray-50 rounded-[2rem] border-2 border-dashed border-gray-200 p-20 text-center">
            ...
            <Link href="/invitations" className="text-emerald-600 font-black text-sm underline">Start by inviting someone</Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {groups.map((group) => (
              <div 
                key={group.id} 
                className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-xl hover:border-emerald-500 transition-all group"
              >
